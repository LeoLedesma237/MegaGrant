...
# Create a list by topographical location
topography.list <- list(select(data2, frontal_delta:frontal_beta),
                        select(data2, temporal_delta:temporal_beta),
                        select(data2, parietal_delta:parietal_beta),
                        select(data2, occipital_delta:occipital_beta))


# Create a separate list with the descriptives we need
topography.descriptives.list <- list()

for(ii in 1:length(topography.list)) {
  
frequency.bands.descriptives <- topography.list[[ii]] %>%
  describe() %>%
  data.frame() %>%
  round(2) %>%
...
frequency.bands.descriptives$`Frequency Band` <- c("Delta (0.5-3.75Hz)",
                                       "Theta (4-7.75Hz)",
                                       "Alpha (8-12Hz)",
                                       "Beta (12-30Hz)")
...
# Save it in a list
topography.descriptives.list[[ii]] <- frequency.bands.descriptives
}


# Print out the descriptive tables
topography.descriptives.list[[1]] %>%
 kbl(caption = "Descriptive Statistics of Power from each Frequency Band (Frontal)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)

topography.descriptives.list[[2]] %>%
 kbl(caption = "Descriptive Statistics of Power from each Frequency Band (Temporal)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
topography.descriptives.list[[3]] %>%
 kbl(caption = "Descriptive Statistics of Power from each Frequency Band (Parietal)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
topography.descriptives.list[[4]] %>%
 kbl(caption = "Descriptive Statistics of Power from each Frequency Band (Occipital)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
# Have all frequency band x topography vectors as a data frame
frequency.bands.top <- do.call(cbind,topography.list)


# Plot the histograms of all continuous variables
continous.predictors.outcome <- cbind(continuous.predictors, topography.list) 
...
continous.predictors.outcome$scores <- continous.predictors.outcome[,ii]
...
# Print the continues predictors/covariates
ggarrange(histograms.list[[1]], histograms.list[[2]], histograms.list[[3]])

# Print the frontal graphs
ggarrange(histograms.list[[4]], histograms.list[[5]], histograms.list[[6]],
          histograms.list[[7]])
          
# Print the temporal graphs
ggarrange(histograms.list[[8]], histograms.list[[9]], histograms.list[[10]],
          histograms.list[[11]])

# Print the parietal graphs
ggarrange(histograms.list[[12]], histograms.list[[13]], histograms.list[[14]],
          histograms.list[[15]])

# Print the occipital graphs
ggarrange(histograms.list[[16]], histograms.list[[17]], histograms.list[[18]],
          histograms.list[[19]])
```

- The Frequency Bands (Outcome Measures) do not look parametric. Thus we will log transform the variables to see if that will help with the distribution of scores.  

- Because there is a value of 0 for total spelling error, we replaced them to .1 to prevent getting infiniity as a value for the log transformation. This does not change the ranks of the 0's in the vector since the next lowest score is 1.

```{r visualize all variables as histograms log transformed, out.width= "90%"}
# Remove the scores variable from above
continous.predictors.outcome <- select(continous.predictors.outcome, - scores)

# Convert 0's to .1
continous.predictors.outcome$Total_SpellingError <- ifelse(continous.predictors.outcome$Total_SpellingError == 0,
                                                           .1,
                                                           continous.predictors.outcome$Total_SpellingError )


# log transform the data
log_continous.predictors.outcome <- as.data.frame(lapply(continous.predictors.outcome, log)) 

# Change the names
names(log_continous.predictors.outcome) <- paste("log_",names(log_continous.predictors.outcome),sep="")

# Plot them all as separate histograms
histograms.log.list <- list()

for(ii in 1:(length(log_continous.predictors.outcome))) {

log_continous.predictors.outcome$scores <- log_continous.predictors.outcome[,ii]
  
histograms.list[[ii]] <- log_continous.predictors.outcome %>%
    ggplot(aes (x = scores)) +
    geom_histogram(fill = "white",
                   color = "black",
                   bins = 20) +
    labs(title = paste(names(log_continous.predictors.outcome[ii]))) +
    theme(plot.title = element_text(size = 14, hjust = 0.5)) +
  theme_classic()
...
```{r descriptive statistics of log power spectrum}
# Print the predictors/covariates in general
predictors.log.descriptives <- select(log_continous.predictors.outcome, log_Total_SpellingError:log_Raw.Scores) %>%
  describe() %>%
  data.frame() %>%
  round(2) %>%
  select(` ` =vars,
...
predictors.log.descriptives$` ` <- c("Spelling Error", "Age", "CFIT Raw Scores")

# Remove the row names
row.names(predictors.log.descriptives) <- NULL


# Print out the predictor descriptive tables
predictors.log.descriptives %>%
 kbl(caption = "Descriptive Statistics of Log Power Predictors",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
# Create a list by topographical location
topography.log.list <- list(
                        select(log_continous.predictors.outcome, log_frontal_delta:log_frontal_beta),
                        select(log_continous.predictors.outcome, log_temporal_delta:log_temporal_beta),
                        select(log_continous.predictors.outcome, log_parietal_delta:log_parietal_beta),
                        select(log_continous.predictors.outcome, log_occipital_delta:log_occipital_beta))


# Create a separate list with the descriptives we need
topography.log.descriptives.list <- list()

for(ii in 1:length(topography.log.list)) {
  
frequency.bands.descriptives <- topography.log.list[[ii]] %>%
  describe() %>%
  data.frame() %>%
  round(2) %>%
  select(`Frequency Band` = vars,
...
# Save it in a list
topography.log.descriptives.list[[ii]] <- frequency.bands.descriptives
}



# Print out the descriptive tables
topography.log.descriptives.list[[1]] %>%
 kbl(caption = "Descriptive Statistics of Log Power from each Frequency Band (Frontal)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)

topography.log.descriptives.list[[2]] %>%
 kbl(caption = "Descriptive Statistics of Log Power from each Frequency Band (Temporal)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
topography.log.descriptives.list[[3]] %>%
 kbl(caption = "Descriptive Statistics of Log Power from each Frequency Band (Parietal)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
topography.log.descriptives.list[[4]] %>%
 kbl(caption = "Descriptive Statistics of Log Power from each Frequency Band (Occipital)",
      align = "lccc") %>%
  kable_classic_2(full_width = FALSE,
                  html_font = "Times New Roman") %>%
  row_spec(0,bold=TRUE)
...
# Create datasets by topography
frontal.df <- log_continous.predictors.outcome %>%
  select(log_Total_SpellingError:log_Raw.Scores, log_frontal_delta:log_frontal_beta)

temporal.df <- log_continous.predictors.outcome %>%
  select(log_Total_SpellingError:log_Raw.Scores, log_temporal_delta:log_temporal_beta)

parietal.df <- log_continous.predictors.outcome %>%
  select(log_Total_SpellingError:log_Raw.Scores, log_parietal_delta:log_parietal_beta)

occipital.df <- log_continous.predictors.outcome %>%
  select(log_Total_SpellingError:log_Raw.Scores, log_occipital_delta:log_occipital_beta)


# Create a list for all of these datasets to be correlated
topography.df.list <- list(frontal.df,
                                temporal.df,
                                parietal.df,
                                occipital.df)


topography.names <- c("frontal", "temporal", "parietal", "occipital")
...
for(ii in 1:4) {
  ...
  current.variables <- topography.df.list[[ii]]
  ...
  correlation.matrix.list[[ii]] <- correlation.matrix.df %>%
    kbl(caption = paste("Correlation matrix of our continuous predictors and",names(topography.names[ii]))) %>%
...
scatterplot.list[[ii]]  <- plot(topography.df.list[[ii]])
...
# Convert data finally into long format
x <- topography.df.list[[1]]
