...
rbind(
  ...
  theme_classic() +
  labs(title = "General distribution of Power by Type") +
...
rbind(
  ...
  ggplot(aes(x = Topography, y = Power, fill = Frequency_Band)) +
  #geom_violin() +
  geom_boxplot() +
  facet_grid(Topography~Power_Type, scales = "free") +
  coord_flip() +
  labs(title = "Absolute, Mean Absolute and Relative Power Across\nTopography and Frequency Bands in Eyes Open") +
  theme(plot.title = element_text(hjust = 0.5))
...
for(ii in 1:length(data_list)){ 
  ...
  plot_list[[ii]] <- current_dat %>%
    ...
    facet_wrap(Topography ~ Frequency_Band, scales = "free_y") +
    ...
    scale_color_manual(values = c("#000000", "#B3B3B3", "#D9D9D9", "blue"),
