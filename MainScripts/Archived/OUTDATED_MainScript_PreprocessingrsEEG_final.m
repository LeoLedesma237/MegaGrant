...
% 9) Remove line noise (US = 60 Hz) (novel)
%
...
%   them in more detial. The robust detrending was designed to basically
%   replaced a high-pass filter. This is because the later removes all
%   signal in that frequency, both low and high. Specifically, at the
%   specified high-pass Hz, the amplitude for that regions is reduced by
...
%   by diving the work using multiple cores. It is a finicky process in which
...
beforeICA_html = strrep(beforeICA_ext, '.mat', '.html');
   ...
   % Run parfor loop 
    parfor iter = batchIndices
        try
            ...
            one_parthtml = [name beforeICA_html];
            ...
            cfg_down = [];
            cfg_down.resamplefs = 500;          % New target sampling rate 
            ...
            cfg.removebadchann.zthresh  = 5; % 4 robust SD higher freq than other channels 
            ...
            cfg.removebadchann.blinkchans = {'Fp1', 'Fp2', 'Fz', 'F4', 'F3', 'AF8', 'AF7', 'AF3', 'AF4', 'AFz'}; 
...
myCluster = parcluster('local');
delete(myCluster.Jobs);         % Clears old crash files
myCluster.NumWorkers = 8;       % More than this will cause parfor to fail (ICA)
parpool(myCluster);             % Starts the cluster with specified number of workers
fprintf('Successfully started clean pool with %d workers\n', myCluster.NumWorkers);
...
for batch = 1:numBatches
    ...
    % Every 5th batch kill restart parallel processing
    if mod(batch, 5) == 0
        disp('restarting parallel processing to save up memory')
        pause(5);
        delete(gcp('nocreate'));        % Kills any old pool (12 workers or whatever)                                      
   ...
   % Run parfor loop 
    parfor iter = batchIndices
        try
            ...
            % Same as above delete error and html files if they already exists
            if isfile(error_mat); delete(error_mat); end 
            if isfile(error_csv); delete(error_csv); end 
            if isfile(html_full); delete(html_full); end 
            ...
            %%%%%%%%% PART 9: ICA AND ARTIFACT COMPONENT REMOVAL %%%%%%%%%%%
            ...
            stage = 'Part 9: ICA was sucessful';
            temp_dat = data_referenced;
            ...
            cfg.compblinks.min_blinks = 5; % Changed from 10 to 5 (arbitrary)
            cfg.removebadchann.blinkchans = {'Fp1', 'Fp2', 'Fz', 'F4', 'F3', 'AF8', 'AF7', 'AF3', 'AF4', 'AFz'}; 
            ...
            stage = 'Part 9.1: Rejecting Blink Components was sucessful';
            temp_dat = data_rmvcmp;
    
    
            %%%%%%%%%%%%% PART 10: REMOVE LINE NOISE %%%%%%%%%%%%%%%%
            ...
            stage = 'Part 10: Removing Line Noise was sucessful';
            temp_dat = data_rmvlinenoise;

            % Save as data clean- the cleaning stops here
            data_clean = data_rmvcmp
    
            %%%%%%%% PART 11: INTERPOLATE CHANNELS IN NOISY SEGMENTS (AGAIN) %%%%%%%%%%
  
%             % Create a structure to interpolate noisy channels within segments
%             cfg = [];
%             cfg.chansegmentrepair.zthresh1 = 4; % Robust z-score threshold
%             cfg.chansegmentrepair.zthresh2 = 5; % Robust z-score threshold
...
%             cfg.chansegmentrepair.rmvtrials   = 'yes';
%             cfg.chansegmentrepair.badchanprop =  0.20;
%             cfg.chansegmentrepair.indvbadsegplot =  'no';
% 
...
%             stage = 'Part 11: Interpolating bad channels within trials was sucessful; errors past this are related to plots- most likely because the EEG data is very bad';
%             temp_dat = data_clean;
%     
            %%%%%%%%%%%%%%%%%%%%%% RECORD EEG VARIANCE %%%%%%%%%%%%%%%%%%%%%%%
