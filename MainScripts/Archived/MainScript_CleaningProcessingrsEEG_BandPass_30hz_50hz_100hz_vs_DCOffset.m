...
for ii = 1:6 
    ...
    WetrsEEGParameters_30Hz % Loads in 'preprocParams'

    % Override WetrsEEGParameters parameters based on iteration number
    if ii == 1
        % 30 Hz band pass filter
        % DC offset is set to 'no'
        % Original clean EEG name
        % Orginal clean EEG report
    elseif ii == 2
        % 30 Hz band pass filter
         preprocParams.detrend = 'yes';
         preprocParams.fileExtEeg = strrep(preprocParams.fileExtEeg,'30Hz', '30Hz_detrend');
         preprocParams.fileExtQc = strrep(preprocParams.fileExtEeg , '.set', '_QC.csv');
    elseif ii == 3
        preprocParams.filtHigh = 50;
        % DC offset is set to 'no'
        preprocParams.fileExtEeg = strrep(preprocParams.fileExtEeg,'30Hz', '50Hz');
        preprocParams.fileExtQc = strrep(preprocParams.fileExtEeg , '.set', '_QC.csv');
    elseif ii == 4
        preprocParams.filtHigh = 50;
        preprocParams.detrend = 'yes';
        preprocParams.fileExtEeg = strrep(preprocParams.fileExtEeg,'30Hz', '50Hz_detrend');
        preprocParams.fileExtQc = strrep(preprocParams.fileExtEeg , '.set', '_QC.csv');
    elseif ii == 5
        preprocParams.filtHigh = 100;
        % DC offset is set to 'no'
        preprocParams.fileExtEeg = strrep(preprocParams.fileExtEeg,'30Hz', '100Hz');
        preprocParams.fileExtQc = strrep(preprocParams.fileExtEeg , '.set', '_QC.csv');
    elseif ii == 6
        preprocParams.filtHigh = 100;
        preprocParams.detrend = 'yes';
        preprocParams.fileExtEeg = strrep(preprocParams.fileExtEeg,'30Hz', '100Hz_detrend');
        preprocParams.fileExtQc = strrep(preprocParams.fileExtEeg , '.set', '_QC.csv');
    end

    % Location of the sub folder where all processed data will be saved
    % (name of the analysis)
    folder = '30hz_50hz_100hz_vs_DCOffset';
    ...
    % DELETE ME LATER (PICK FIRST 5 FILES; to test)
    eegFiles = eegFiles(1:150);
...
main_QC_dry_name = append(Mega, 'MegaGrant\',folder, '\30hz_50hz_100hz_QC_reports.xlsx');
...
final_outputname = '30hz_50hz_100hz_topographyFBAvgPowWelch.csv'; 
